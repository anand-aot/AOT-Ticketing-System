-- 0007_create_ticket_trigger.sql
-- Drop existing trigger if it exists
DROP TRIGGER IF EXISTS ticket_changes_trigger ON tickets;

-- Create or replace trigger function for ticket changes
...
DECLARE
  ...
  ticket_url TEXT := (SELECT value FROM secrets WHERE key = 'APP_BASE_URL') || '/ticket/' || NEW.id;
  ...
  edge_function_url TEXT := (SELECT value FROM secrets WHERE key = 'EDGE_FUNCTION_URL');
BEGIN
  -- Ensure edge function URL is set
  IF edge_function_url IS NULL THEN
    RAISE EXCEPTION 'Missing EDGE_FUNCTION_URL in secrets table';
  END IF;
